...
def create_category_folders(folder: Path, valid_move_plan: dict[Path, str]):
    ...
    for category in categories:
        ...
        try:
            ...
            print(f"Could not create {category} folder: {e}")
            failed_categories.add(category)

        else:
            created_categories.add(category)

    return failed_categories, created_categories

def create_final_move_plan(valid_move_plan: dict[Path, str], created_categories: set[str]):
    # If any categories failed to be created, this function returns a final cleaned up move plan
    ...
    for file, category in valid_move_plan.items():
        if category in created_categories:
            ...
            final_move_plan[file] = category
    
    return final_move_plan
...
def count_failed_category_files(move_plan: dict[Path, str], failed_categories: set[str]):
    # Returns a count of files affected by category folder creation error

    failed_category_files_counter = 0

    for file, category in move_plan.items():
        if category in failed_categories:
            failed_category_files_counter +=1

    return failed_category_files_counter
...
def main():
    ...
    folder = get_folder()
    move_counter = 0
    failed_categories_files_counter = 0
    ...
    if validate_folder(folder):
        ...
        failed_categories, created_categories = create_category_folders(folder, valid_move_plan)

        if failed_categories:
            
            failed_categories_files_counter = count_failed_category_files(valid_move_plan, failed_categories)
            valid_move_plan = create_final_move_plan(valid_move_plan, created_categories)

        if valid_move_plan:
            move_counter = move_files(valid_move_plan)
            print("Organisation completed!")
            print(f"{move_counter} files moved.")
        else:
            print("No moves could be made. Program finished")

        unmovable_files_no = (conflicting_category_files_counter + 
                              conflicting_name_files_counter + 
                              unsupported_files_counter +
                              failed_categories_files_counter)
        ...
        if unmovable_files_no > 0:
            ...
            if further_information.strip().lower() == "yes":
                ...
                if failed_categories:
                    print(f"The categories: {failed_categories} could not be made due to some OS exception.")
                    print(f"{failed_categories_files_counter} files could not be moved due to this issue.")
